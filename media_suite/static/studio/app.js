...
(() => {
  ...
  async function pickFor(key) {
    const dlg = $("#picker"); const multi = key === "model";
    $("#picker-title").textContent = ({ product: "Product reference", source: "Image to use", model: "Model photo", fabric: "Fabric reference", style: "Style from another product", logo: "Company logo" }[key] || "Choose image") + (multi ? " — tick several, then close" : "");
  ...
  function actionMenu(a, r) {
    ...
    const sel = el("select", { class: "action-select", onchange: async (e) => { const v = e.target.value; e.target.value = ""; if (v) await doAction(v, a, r); } },
      ...
      ...(!isVideo ? [el("option", { value: "remove_bg" }, "Remove background"), el("option", { value: "replace_bg" }, "Replace background…"), el("option", { value: "use_source" }, "Use as source for edit/variants"), el("option", { value: "variants" }, "Make colour variants"), el("option", { value: "versions" }, "Versions"), el("option", { value: "other" }, "Add to another product…")] : []),
  ...
  async function doAction(v, a, r) {
    try {
      ...
      if (v === "remove_bg" || v === "replace_bg") { let background = S.opts.background; if (v === "replace_bg") { const b = await ask({ title: "New background", select: S.tasks.backgrounds.filter((x) => x.id !== "transparent").map((x) => ({ value: x.id, label: x.label })), ok: "Replace" }); if (!b.ok) return; background = b.selected; } const res = await api("/api/studio/run", { method: "POST", body: JSON.stringify({ task: v, product_id: S.product.id, refs: { source_asset_id: a.id }, options: { ...cleanOpts(), background } }) }); notify("success", `Started (${res.provider})`); loadResults(); return; }
  ...
  $("#edit-product").addEventListener("click", async () => {
    ...
    try {
      ...
      $("#ed-qr-box").hidden = true; $("#ed-sku-msg").textContent = "";
      $("#ed-skus").onclick = async () => { try { const r = await api("/api/studio/skus", { method: "POST", body: JSON.stringify({ product_id: S.product.id, apply: false }) }); for (const row of r.rows) { const inp = tb.querySelector(`tr[data-id="${row.variant_id}"] input[data-sku]`); if (inp) inp.value = row.sku; } $("#ed-sku-msg").textContent = `SKUs generated with pattern ${r.pattern} (brand ${r.brand}) — press Save to apply.`; } catch (e) { $("#ed-sku-msg").textContent = e.message; } };
      $("#ed-qr").onclick = async () => { try { const r = await api(`/api/studio/qr/product/${encodeURIComponent(S.product.id)}`); const box = $("#ed-qr-box"); box.hidden = false; box.replaceChildren(); if (!r.pattern) { box.append(el("p", { class: "muted" }, "Set the storefront product URL pattern in Settings first.")); return; } for (const it of r.items) { const q = `/api/studio/qr?${new URLSearchParams({ url: it.url, label: it.sku || it.label })}`; const img = el("img", { alt: it.url, style: "aspect-ratio:1;object-fit:contain" }); fetch(q, { headers: { "X-Saleor-Domain": S.domain, "X-Saleor-Token": S.token } }).then((r) => r.blob()).then((b) => { img.src = URL.createObjectURL(b); }); box.append(el("div", { class: "tile" }, img, el("div", { class: "cap", title: it.url }, it.sku || it.label), el("div", { class: "actions" }, el("button", { class: "btn btn-sm", onclick: async () => { const res = await fetch(q, { headers: { "X-Saleor-Domain": S.domain, "X-Saleor-Token": S.token } }); const blob = await res.blob(); const l = document.createElement("a"); l.href = URL.createObjectURL(blob); l.download = `qr-${(it.sku || it.label).replace(/[^a-z0-9]+/gi, "-")}.png`; l.click(); } }, "Download"), el("a", { class: "btn btn-sm", href: it.url, target: "_blank", rel: "noopener" }, "Open")))); } } catch (e) { $("#ed-sku-msg").textContent = e.message; } };
